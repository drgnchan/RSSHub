        ...
        item: await Promise.all(
            list.toArray().map(async (item) => {
                ...
                if (item.html().search(/dysMiddleResultConItemTitle/g) !== -1) {
                    ...
                    contentUrl = href.startsWith('/') || href.startsWith('.') ? `${url.replace('index.htm', '')}${href.replace('.', '')}` : href;
                    if (!contentUrl.includes('content')) {
                        description = item.find('a').text(); // 忽略获取吹风会的全文
                    } else {
                        description = await ctx.cache.tryGet(contentUrl, async () => {
                ...
                return {
                    ...
                    pubDate: (function() {
                        if (uid !== 'zhengce') {
                            return timezone(parseDate(item.find('span').text()), 0);
                        } else {
                            return timezone(parseDate(item.find('.dysMiddleResultConItemRelevant span:nth-of-type(2)').text()), 0);
                        }
                    })(),
                };
            }),
